...
pnpm build:wizard             # Wizard + its dependencies only

# Test
pnpm test                     # All unit tests (vitest across all packages)
...
# Dev servers
pnpm dev                      # API (port 3007) + wizard (port 5173) concurrently
pnpm dev:api                  # API only
pnpm dev:wizard               # Wizard only (proxies /api → localhost:3007)
```
...
```
TTL (SHACL/OWL ontology) → shacl-core parser → ShaclModel JSON → wizard renders dynamic forms → exports JSON-LD
```
...
**apps/wizard** — React 19 SPA built with Vite 6. Uses TanStack Router (file-based routing), TanStack Query (API state), react-hook-form (dynamic forms), Tailwind CSS 4 (styling), and Radix UI (accessible primitives). `lib/shape-to-fields.ts` converts ShaclModel to `FieldDescriptor[]` which `DynamicField` renders per type. Step-based wizard groups fields by target class.

**apps/e2e** — Playwright E2E tests. Config auto-starts both API and wizard dev servers. Uses `data-testid` selectors.
...
- **Vite handles transpilation** — The wizard uses Vite + SWC for fast builds; `tsc --noEmit` for type-checking only.
...
- **API proxy** — Wizard proxies `/api/*` to `localhost:3007` with path rewrite (strips `/api` prefix).
- **ShaclModel contract** — The `types.ts` interfaces in shacl-core must remain backwards-compatible with the wizard and the external `sl-5-8-asset-tools` pipeline that consumes this API as a submodule.
